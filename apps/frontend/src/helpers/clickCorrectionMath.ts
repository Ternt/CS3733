...
export function customPathHelper(props: findEdgeboundPathProps) {
    ...
    const pn2TL: vec2 = {
        x: props.nodes.filter((nodeItem) => nodeItem.nodeID === secondToLastNode)[0]
            .xcoord,
        y: props.nodes.filter((nodeItem) => nodeItem.nodeID === secondToLastNode)[0]
            .ycoord,
    };
    const pnL: vec2 = {
        x: props.nodes.filter((nodeItem) => nodeItem.nodeID === lastNode)[0]
            .xcoord,
        y: props.nodes.filter((nodeItem) => nodeItem.nodeID === lastNode)[0]
            .ycoord,
    };
    ...
    if (dist2TLToL < dist2TLToP) { // Selected point is past the last pathfinding node
        return lastNode;
    } else {
